...
const MyProfile = () => {
  const {userData,setUserData,token1,backendUrl,loadUserProfileData} = useContext(AppContext)
  ...
  const updateUserProfileData = async() => {
    try{
      ...
      const {data} = await axios.post(backendUrl+'/api/user/update-profile',formData,{headers:{token1}})
      ...
      <div>
        ...
        <div className='grid grid-cols-[1fr_3fr] gap-y-2.5 mt-3 text-neutral-700'>
          ...
          <p className='font-medium'>Address:</p>
          {
            isEdit
              ? <p>
                <input className='bg-gray-100' value={userData.address.line1} type="text" onChange={e => (setUserData(prev => ({ ...prev, address: { ...prev.address, line1: e.target.value } })))} />
                <br />
                <input  className='bg-gray-100' value={userData.address.line2} type="text" onChange={e => (setUserData(prev => ({ ...prev, address: { ...prev.address, line2: e.target.value } })))} />
              </p>
